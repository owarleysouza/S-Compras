...
class Categoria extends StatelessWidget {
  final List produtosG;
  final List produtosLeH;
  final List produtosF;

  Categoria(
      {@required this.produtosG,
      @required this.produtosLeH,
      @required this.produtosF});
  ...
  Widget build(BuildContext context) {
    return Container(
      child: Column(
        children: <Widget>[
          Text("Grosso"),
          ...produtosG.map((produto) => produto.categoria == "Grosso"
              ? ProdutoTemplate(
                  nome: produto.nome, quantidade: produto.quantidade)
              : Text("")),
          Text("Limpeza e Higiene"),
          ...produtosLeH.map((produto) => produto.categoria == "LeH"
              ? ProdutoTemplate(
                  nome: produto.nome, quantidade: produto.quantidade)
              : Text("")),
          Text("Frios"),
          ...produtosF.map((produto) => produto.categoria == "Frios"
              ? ProdutoTemplate(
                  nome: produto.nome, quantidade: produto.quantidade)
              : Text("")),
        ],
